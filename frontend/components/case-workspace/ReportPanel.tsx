...
function LegacyDraft({ output, busy, request }: { output: AIOutput; busy: string; request: Request }) {
  ...
  return <section className="workspace-card"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><EntityCode kind="report" id={output.id} /><h3 className="font-semibold text-ink">{output.title}</h3></div><StatusBadge status={output.review_status} /></div><textarea className="mt-4 min-h-96 w-full rounded-md border border-line px-3 py-3 font-mono text-sm leading-6 focus:border-court" value={draft} onChange={(event) => setDraft(event.target.value)} /><input className="mt-3 w-full rounded-md border border-line px-3 py-2 text-sm focus:border-court" value={reason} onChange={(event) => setReason(event.target.value)} /><div className="mt-3 flex flex-wrap gap-2"><button className="button-primary" disabled={Boolean(busy)} onClick={() => submit("接受", output.content)}>接受初稿</button><button className="button-secondary" disabled={Boolean(busy)} onClick={() => submit("修改", draft)}>保存人工修改</button><button className="button-secondary" disabled={Boolean(busy)} onClick={() => submit("驳回")}>驳回</button></div></section>;
}
...
export function ReportPanel({ caseId, workspace, busy, request }: { caseId: number; workspace: CaseWorkspace; busy: string; request: Request }) {
  ...
  return <section className="space-y-5"><PanelHeading title={step.title} description={step.description} action={<button className="button-primary" disabled={Boolean(busy) || !ready} onClick={() => request(`/cases/${caseId}/legal-analysis-report`)}><FileText size={16} />{report ? "重新生成报告" : "生成法律分析报告"}</button>} />{!ready && <div className="feedback-state border-[var(--warning)] bg-[var(--warning-bg)] text-[var(--warning)]">至少完成一项当前版本法律分析并通过人工复核后，才能生成报告。</div>}{report ? <article className="workspace-card"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><EntityCode kind="report" id={report.id} /><h3 className="font-semibold text-ink">{report.title}</h3></div><p className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500"><span className="version-chip version-chip-court"><span className="text-slate-400">报告</span><span className="font-semibold">V{report.version}</span></span><span className="version-chip"><span className="text-slate-400">事实</span><span className="font-semibold">V{report.fact_version}</span></span><span className="version-chip"><span className="text-slate-400">争点</span><span className="font-semibold">V{report.issue_version}</span></span></p></div><StatusBadge status={report.review_status || "已完成"} /></div><div className="mt-5 border-t border-line pt-5 whitespace-pre-wrap text-sm leading-8 text-slate-700">{report.reviewed_content || report.content}</div></article> : <EmptyState title="尚未生成法律分析报告" description="完成分析复核后，点击右上角按钮生成当前版本报告。" />}</section>;
}
